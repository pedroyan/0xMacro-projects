...
error DelegateAlreadySet(address member, address delegate);
...
contract CollectorDao is CollectorDaoEIP712 {
	...
	struct DaoMember {
		...
		uint64 votingPower;
		/// @notice The address of the delegate of this member.
		address delegate;
	}
	...
	function setDelegate(address delegate) external {
		...
		if (delegate != address(0)) delegators[delegate].push(msg.sender);
	}
	...
	function purchaseMembership() external payable {
		...
		// Create new member
		members[msg.sender] = DaoMember({
			joinedAtProposalNumber: latestProposalNumber,
			votingPower: 1,
			delegate: address(0)
		});
		memberCount++;
