...
import { IcoPhase, ICO_MAX_CONTRIBUTION_LIMIT } from './helpers'
...
describe('ICO', () => {
  ...
  const MAX_CONTRIBUTION_LIMIT = ICO_MAX_CONTRIBUTION_LIMIT
  ...
  describe('SEED Phase', () => {
    ...
    it('it should revert ETH withdrawal on the SEED Phase', async () => {
      // Arrange
      const { ico, deployer } = await loadFixture(setupFixture)
      await ico.connect(allowlistedInvestors[0]).contribute({ value: ethers.utils.parseEther('1') })

      // Act
      const promise = ico.connect(deployer).withdraw()
  ...
  describe('GENERAL Phase', () => {
    ...
    it('it should revert ETH withdrawal on the GENERAL Phase', async () => {
      // Arrange
      const { ico, deployer } = await loadFixture(setupGeneralPhase)
  ...
  describe('OPEN Phase', () => {
    ...
    it('it should allow owner to withdraw ETH to treasury', async () => {
      // Arrange
      const { ico, treasury } = await loadFixture(setupOpenPhase)
      const contribution = MAX_CONTRIBUTION_LIMIT
      await ico.connect(alice).contribute({ value: contribution })
      ...
      // Assert
      expect(treasuryBalanceAfter.sub(treasuryBalanceBefore)).to.equal(contribution)
    })

    it('it should reject non-owner to withdraw ETH to treasury', async () => {
      // Arrange
      const { ico, treasury } = await loadFixture(setupOpenPhase)
      const contribution = MAX_CONTRIBUTION_LIMIT
      await ico.connect(alice).contribute({ value: contribution })
      ...
      // Act
      const promise = ico.connect(alice).withdraw()
  ...
  describe('Attacks', () => {
    ...
    describe('ETH Force-Feeding', async () => {
      it('should not account for force-fed ETH', async () => {
        ...
        // Act
        await forceFeeder.connect(deployer).forceFeed({ value: ethers.utils.parseEther('1') })
      ...
      it('should not take into account force-fed ETH in the phase limits', async () => {
        ...
        // Act
        await forceFeeder.connect(deployer).forceFeed({ value: SEED_INDIVIDUAL_LIMIT }) // Contribute 1_500 ETH, but not accounted for the limit
      ...
      it('should not skew token redemption ratios for force-fed ETH', async () => {
        ...
        // Act
        await ico.connect(alice).contribute({ value: contribution })
        await forceFeeder.connect(bob).forceFeed({ value: ethers.utils.parseEther('100') })
        await ico.connect(alice).redeemTokens()
